...
void JobManager::showDashboard() const
{
    ...
    int notApplied = 0, applied = 0, oaScheduled = 0, oaCompleted = 0;
    int technicalInterview = 0, hrInterview = 0, selected = 0;
    int rejected = 0, offerAccepted = 0, offerDeclined = 0;

    for (const Job &job : jobs)
    {
        string status = job.getStatus();

        if (status == "Not Applied") notApplied++;
        else if (status == "Applied") applied++;
        else if (status == "OA Scheduled") oaScheduled++;
        else if (status == "OA Completed") oaCompleted++;
        else if (status == "Technical Interview") technicalInterview++;
        else if (status == "HR Interview") hrInterview++;
        else if (status == "Selected") selected++;
        else if (status == "Rejected") rejected++;
        else if (status == "Offer Accepted") offerAccepted++;
        else if (status == "Offer Declined") offerDeclined++;
    }
...
string JobManager::getJobsJson() const
{
    ostringstream oss;
    oss << "[";
...
string JobManager::getDashboardJson() const
{
    int notApplied = 0, applied = 0, oaScheduled = 0, oaCompleted = 0;
    int technicalInterview = 0, hrInterview = 0, selected = 0;
    int rejected = 0, offerAccepted = 0, offerDeclined = 0;
    ...
    oss << "{"
        ...
        << "\"applied\":" << applied << ","
        << "\"oaScheduled\":" << oaScheduled << ","
        << "\"oaCompleted\":" << oaCompleted << ","
...
bool JobManager::addJobFromApi(const Job &newJob)
{
...
bool JobManager::updateStatusById(int id, const string &newStatus)
{
    for (Job &job : jobs)
    {
        if (job.getJobId() == id)
...
bool JobManager::deleteById(int id)
{
    for (auto it = jobs.begin(); it != jobs.end(); ++it)
    {
        if (it->getJobId() == id)
